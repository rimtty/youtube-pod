...
- ライブラリ保存時の音声正規化（フラグメント化M4A→フラットM4A、SHA-256記録）、バックフィル、Watch転送時の正規化済みファイル再利用
...
- 2026-09-13 正規化前倒し後: Debug全テスト182件（うちネットワーク実動画4件は通常実行ではスキップ）、失敗0件。Watch側Unit回帰テストも失敗0件
...
- ライブラリ保存直後に`LibraryAudioOptimizer`がフラグメント化M4AをフラットM4Aへ再多重化（AACの再エンコードなし）し、`SavedAudio.audioContentSHA256`／`audioNormalizationVersion`を記録することを確認。正規化後ファイルは元ファイルと同じ時刻からデコードしたPCMが一致し、iPhoneの再生位置・シークバーに影響しない
- ダウンロード行は「音声を確認中」→「音声を最適化中 NN%」→「保存済み」と遷移し、最適化中の個別キャンセル／バックグラウンド移行では保存済み音声を削除しない。最適化はダウンロードの直列スロットを保持するため、連続ダウンロード時は各件の最適化分だけ次の取得が待つ（yt-dlpとのディスクI/O競合を避ける意図的な仕様）
